...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div>
            ...
            <h1 class="text-xl font-bold text-slate-800 tracking-tight mt-1.5">Tambah Foto Galeri</h1>
            <p class="text-xs text-slate-400">Unggah foto baru beserta judul dan deskripsi singkat.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.galeri.store') }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm flex flex-col justify-between">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="judul" id="judul" value="{{ old('judul') }}" placeholder="Masukkan judul foto..." 
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="5" placeholder="Tulis deskripsi singkat tentang foto ini..." 
                                  class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-xs text-slate-700 placeholder:text-slate-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition resize-none">{{ old('deskripsi') }}</textarea>
                    </div>
                </div>

                <div class="flex items-center gap-3 pt-6 border-t border-slate-100 mt-6 lg:mt-0">
                    <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition text-xs text-center cursor-pointer">
                        Simpan Foto
                    </button>
                    <a href="{{ route('admin.galeri.index') }}" class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-2.5 px-4 rounded-xl transition text-xs text-center cursor-pointer">
                        Batal
                    </a>
                </div>
            </div>

            <!-- Right Card: Photo Upload & Preview -->
            <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-sm flex flex-col">
                ...
                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-3">Pilih / Seret Foto <span class="text-red-500">*</span></label>
                
                <!-- Hidden Real File Input -->
                <input type="file" name="foto" id="foto-input" accept="image/*" class="hidden" required>

                <!-- Drag and Drop Dropzone -->
                <div id="dropzone" class="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 hover:border-blue-400 rounded-2xl p-8 text-center cursor-pointer bg-slate-50/50 hover:bg-blue-50/20 transition-all duration-300 min-h-[300px] relative overflow-hidden group">
                    
                    <!-- Upload Prompt State -->
                    <div id="upload-prompt" class="space-y-3 flex flex-col items-center">
                        <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center border border-slate-100 shadow-sm group-hover:scale-105 transition-transform duration-300">
                            <svg class="w-6 h-6 text-slate-400 group-hover:text-blue-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.233-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z" />
                            </svg>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-slate-700">Tarik & lepas file foto di sini</p>
                            <p class="text-[11px] text-slate-400 mt-1">atau klik untuk memilih dari komputer</p>
                        </div>
                        <p class="text-[10px] text-slate-400/80 font-medium">Format: JPG, JPEG, PNG, GIF (Maksimal 2MB)</p>
                    </div>

                    <!-- Preview State (hidden initially) -->
                    <div id="preview-container" class="hidden absolute inset-0 w-full h-full p-2 bg-white">
                        <div class="relative w-full h-full rounded-xl overflow-hidden group/preview bg-slate-50 flex items-center justify-center">
                            <img id="image-preview" src="#" alt="Pratinjau Foto" class="w-full h-full object-cover">
                            
                            <!-- Overlay actions on hover -->
                            <div class="absolute inset-0 bg-slate-950/40 opacity-0 group-hover/preview:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                <button type="button" id="change-photo-btn" class="px-3.5 py-2 bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-xl shadow-md transition-all duration-200 hover:scale-105 cursor-pointer">
                                    Ganti Foto
                                </button>
                                <button type="button" id="remove-photo-btn" class="p-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-md transition-all duration-200 hover:scale-105 cursor-pointer">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            dropzone.addEventListener('click', (e) => {
                // Prevent click bubbling from preview action buttons
