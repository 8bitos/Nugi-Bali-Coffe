...
<div class="mx-auto max-w-3xl">
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="POST" action="{{ route('admin.menu.store') }}" enctype="multipart/form-data" class="space-y-5" id="menuForm">
            @csrf

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div class="md:col-span-2">
                    <label class="mb-2 block text-sm font-semibold text-slate-700">Nama Menu</label>
                    <input name="nama_menu" value="{{ old('nama_menu') }}" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100" placeholder="Contoh: Nasi Goreng" required>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <select name="kategori" id="kategoriSelect" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100" required>
                            ...
                            @foreach($kategori as $k)
                                <option value="{{ $k->nama }}" @selected(old('kategori') === $k->nama)>{{ $k->nama }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-2 text-xs text-slate-500">Kategori dipakai untuk filter dan grouping menu.</div>
                </div>
...
<div id="kategoriModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40 p-4">
    <div class="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl ring-1 ring-black/10">
        ...
        <div class="mt-4 space-y-3">
            ...
            <input id="kategoriNama" type="text" class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100" placeholder="Contoh: Minuman">
